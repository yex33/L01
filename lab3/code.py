...
for n in n_range:
    ...
    sel = selection_sort
    ins = insertion_sort
    qui_time = min(timeit.repeat("qui(L)", globals=globals(), repeat=5, number=1))
    qui_res.append(qui_time)
    bub_time = min(timeit.repeat("bub(L2)", globals=globals(), repeat=5, number=1))
    bub_res.append(bub_time)
    sel_time = min(timeit.repeat("sel(L3)", globals=globals(), repeat=5, number=1))
    sel_res.append(sel_time)
    ins_time = min(timeit.repeat("ins(L4)", globals=globals(), repeat=5, number=1))
